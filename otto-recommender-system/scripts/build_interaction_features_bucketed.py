...
from otto_recommender.interaction_features import write_interaction_features_part


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build candidate-level interaction features by bucket.")
    ...
    parser.add_argument("--output-dir", default="artifacts/features/interaction_features_parts")
    parser.add_argument("--stats-output", default="artifacts/reports/interaction_features_stats.csv")
...
def main() -> None:
    ...
    for bucket in range(args.bucket_start, end):
        ...
        output_path = output_dir / f"interaction_features_part_{bucket:03d}.parquet"
        rows = write_interaction_features_part(
        ...
        stats.append({"bucket": bucket, "rows": rows, "path": str(output_path)})
        print(f"bucket={bucket} rows={rows:,} -> {output_path}")

    stats_path = Path(args.stats_output)
    stats_path.parent.mkdir(parents=True, exist_ok=True)
    pl.DataFrame(stats).write_csv(stats_path)
